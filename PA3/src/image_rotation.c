...
if(executed == pics_changed){
printf("Worker ends %d\n", pics_changed);
printf("Queue size is %d\n",executed);
printf("Next remove is %d\n",next_remove);
printf("%d\n", queue_size);
// pthread_mutex_lock(&queue_lock);
...
int main(int argc, char* argv[]){
	...
	if(pthread_create(&processing_thread, NULL, (void*)processing, &pargs) != 0){
	...
	for (int i = 0; i < num_worker_threads; i++){
		wthreads[i] = i;
		if(pthread_create(&worker_threads[i], NULL, (void*)worker, &wthreads[i]) != 0){
